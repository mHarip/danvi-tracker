...
export default function HistoryPage() {
    ...
    const groupedByDate = (data, key = "startTime") =>
        data.reduce((acc, item) => {
            const dateKey = item[key] || item.time || item.createdAt;
    ...
    const getDailyTotalOz = (records) =>
        records
            .filter(
                (r) =>
                    ["pumped", "formula", "bottle"].includes(r.type?.toLowerCase()) &&
                    r.amount
            )
    ...
    const getDailyTotalSleep = (records) =>
        records
            .reduce(
                (sum, r) =>
                    sum +
                    (new Date(r.endTime) - new Date(r.startTime)) / 3600000,
                0
            )
            .toFixed(1);
    ...
    const renderTable = (title, data, columns, typeKey = "feedings") => {
        const grouped = groupedByDate(
            data,
            typeKey === "diapers" ? "time" : "startTime"
        );
        ...
        return (
            <div className="col-lg-6">
                <div className="card mb-4 shadow-sm border-1">
                    ...
                    {/* Always show filters; if no results, show message below */}
                    {dates.length === 0 ? (
                                                        ...
                                                        <tr key={item.id}>
                                                            {typeKey === "feedings" && (
                                                                <>
                                                                    ...
                                                                    <td>{item.startTime ? new Date(item.startTime).toLocaleTimeString([], {
                                                                        hour: "2-digit",
                                                                        minute: "2-digit"
                                                                    }) : "-"}</td>
                                                                    <td>{item.endTime ? new Date(item.endTime).toLocaleTimeString([], {
                                                                        hour: "2-digit",
                                                                        minute: "2-digit"
                                                                    }) : "-"}</td>
                                                                </>
                                                            ...
                                                            {typeKey === "diapers" && (
                                                                <>
                                                                    <td>{item.type ? item.type.charAt(0).toUpperCase() + item.type.slice(1) : "-"}</td>
                                                                    <td>{item.time ? new Date(item.time).toLocaleTimeString([], {
                                                                        hour: "2-digit",
                                                                        minute: "2-digit"
                                                                    }) : "-"}</td>
                                                                </>
                                                            ...
                                                            <td className="actions-dropdown">
                                                                <div className="dropdown">
                                                                    <button
                                                                        ...
                                                                        aria-label="Actions"
                                                                    >
                                                                        <img src="/edit.png" alt="Actions" width={18}
                                                                             height={18}/>
                                                                    </button>
